	...
	###############
		...
		code <- nimble::nimbleCode({
			...
			for (i in 1:numCounties) {
				...
				# logit(psi[i]) <- psi_car[i] + psi_beta1 * predictor1[i] + psi_beta1pow2 * (predictor1[i]^2) + psi_beta2 * predictor2[i] + psi_beta2pow2 * (predictor2[i]^2) + psi_beta3 * predictor3[i] * psi_beta3pow2 * (predictor3[i]^2)
				# psi_island[i] ~ dnorm(psi_islandMean, tau=0.001)
